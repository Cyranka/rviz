...
causes_by_year %>% filter(fire_cause == "Human") %>%
  ggplot(aes(x = year, y = percent*100)) + geom_line(size = 1.1, alpha = 0.5) + 
  geom_smooth(se = FALSE, color = "red", size = 1.1) + theme_bw() + 
  labs(x = "Year", y ="% of total fires",
       title = "% of total fires caused by humans by year",
...
decades %>%
  ...
  labs(x = "Year", color = "Decade", y = "Mean log acres of fire",
       title = "Mean log acres of fire of human-related incidents",
       subtitle = "There has been a visible decrease in the acreage of fires caused by humans \nSmooth line in black",
       caption = "Tidy Tuesday Week 21: California Fires")
