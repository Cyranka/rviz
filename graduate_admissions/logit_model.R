...
library(tidyverse); library(caret);library(arm);library(hrbrthemes)
...
avg_sim_gre %>%
  ...
  labs(x = "GRE score",
       ...
       title = "Predicted probabilities of admission into graduate school by GRE Score",
  ...
  scale_x_continuous(limits = c(290, 340),
...
lm_1$finalModel %>% broom::tidy() %>% 
  ...
  ggplot(aes(x = reorder(term, estimate), y = estimate)) + geom_point(size =2, color = "black") + 
  coord_flip() +
