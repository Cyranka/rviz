...
k%>%
  ggplot(aes(x = year, y = turnout, color = state_factor)) + geom_line(show.legend = FALSE, size = 3) + 
  ...
  labs(x = "Year",
       y ="Turnout in %",
       title = "Trends in turnout in high and low turnout states: Midterm elections",
       subtitle = "Top and bottom states selected on the basis of their average turnout between 1982 and 2014",
       caption = "Tidy tuesday week 28: turnout in the United States\nMissing numbers were replaced with the median for that year") + 
  scale_x_continuous(breaks = seq(1982, 2014, 4)) + 
  ...
  scale_color_manual(values = c("chartreuse4","chartreuse4","chartreuse4","darkorange2","darkorange2","darkorange2")) + 
